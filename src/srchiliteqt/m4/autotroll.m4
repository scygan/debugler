...
# AT_WITH_QT([QT_suffix], [QT_modules], [QT_config], [QT_misc])
# ------------------------------------------------
# Enable Qt support and add an option --with-qt to the configure script.
#
# The suffix QT_suffix specifies the suffix to append to qt binaries:
# this is useful for selecting a specific qt installation, e.g., -qt3 or -qt4
#
...
dnl Memo: AC_ARG_WITH(package, help-string, [if-given], [if-not-given])
  ...
  AC_ARG_WITH([qt-dir],
              [AS_HELP_STRING([--with-qt-dir],
                 [Qt dir @<:@Look in PATH and /usr/local/Trolltech@:>@])],
              [QT_QTDIR=$withval], [QT_QTDIR=])
         AC_SUBST([QT_QTDIR])
  ...
  # build binary names using the possible suffix
  QMAKE_BIN="qmake$1"
  MOC_BIN="moc$1"
  UIC_BIN="uic$1"
  RCC_BIN="rcc$1"

  echo "using QT_PATH=$QT_PATH"
  echo "using QT_QTDIR=$QT_QTDIR"
  
  if test x"QT_QTDIR" != x; then
     # override also QT_DIR
     QT_DIR=$QT_QTDIR
     AC_SUBST([QT_DIR])
  ...
  # Find qmake.
  AC_PATH_PROGS([QMAKE], [$QMAKE_BIN qmake], [missing], [$QT_DIR:$QT_PATH:$PATH:$tmp_qt_paths])
  if test x"$QMAKE" = xmissing; then
    AC_MSG_ERROR([Cannot find qmake in your PATH. Try using --with-qt.])
  fi

  # Find moc (Meta Object Compiler).
  AC_PATH_PROGS([MOC], [$MOC_BIN moc], [missing], [$QT_PATH:$PATH:$tmp_qt_paths])
  ...
  AC_PATH_PROGS([UIC], [$UIC_BIN uic], [missing], [$QT_PATH:$PATH:$tmp_qt_paths])
  if test x"$UIC" = xmissing; then
    AC_MSG_ERROR([Cannot find uic (User Interface Compiler) in your PATH. Try using --with-qt.])
  fi

  # Find rcc (Qt Resource Compiler).
  # for rcc, since it's not in qt3, also check the name rcc without suffix
  AC_PATH_PROGS([RCC], [$RCC_BIN rcc], [false], [$QT_PATH:$PATH:$tmp_qt_paths])
...
m4_ifval([$2], [_AT_TWEAK_PRO_FILE([QT], [$2])])

dnl Tweak the value of CONFIG in the .pro if have been given a 2nd arg.
m4_ifval([$3], [_AT_TWEAK_PRO_FILE([CONFIG], [$3])])
...
[ # Add the extra-settings the user wants to set in the .pro
  ...
  # QMAKE_ARGS are passed to qmake (e.g., "CONFIG += debug"
  echo "$as_me:$LINENO: Invoking $QMAKE $QMAKE_ARGS on $pro_file" >&AS_MESSAGE_LOG_FD
  sed 's/^/| /' "$pro_file" >&AS_MESSAGE_LOG_FD

  AC_MSG_CHECKING([Invoking $QMAKE $QMAKE_ARGS])

  if $QMAKE $QMAKE_ARGS; then :; else
    AC_MSG_ERROR([Calling $QMAKE failed.])
  fi

  # an attempt, not working, to deal with problems with backslashes in windows
  #echo "preprocessing Makefile"
  #echo -n "s/[a-zA-Z]:.*\/moc\.exe/" > temp_sed_file.sed
  #echo -n "$MOC" | sed 's/\//\\\//g' >> temp_sed_file.sed
  #echo "/" >> temp_sed_file.sed
  
  #cat temp_sed_file.sed
  #sed -f temp_sed_file.sed Makefile.Debug > foobar.txt
  #cp -f foobar.txt Makefile.Debug
...
instead" >&AS_MESSAGE_LOG_FD
  ...
  if test x"$at_cv_qt_build" = xko; then
    ...
    AC_MSG_ERROR([In windows this might be due to backslashes of binaries])
  fi
  ...
  # this version removes $( ) variables
  qt_remove_sed_filter='s///;
                 s/$(SUBLIBS)//g;
                 s/^ *//;
                 s/\$(\(@<:@A-Z_@:>@@<:@A-Z_@:>@*\))//g'
  ...
  # for windows the debug version generates executables that do not
  # require the dlls?
  if test -f Makefile.Debug; then
    at_mfile='Makefile.Debug'
  else
    at_mfile='Makefile'
  fi
  ...
  [at_cv_env_QT_CFLAGS=`sed "/^CFLAGS@<:@^A-Z@:>@*=/!d;$qt_remove_sed_filter" $at_mfile`])
  AC_SUBST([QT_CFLAGS], ["$at_cv_env_QT_CFLAGS $at_cv_env_QT_DEFINES"])

  # Find the CXXFLAGS of Qt.
  AC_CACHE_CHECK([for the CXXFLAGS to use with Qt], [at_cv_env_QT_CXXFLAGS],
  [at_cv_env_QT_CXXFLAGS=`sed "/^CXXFLAGS@<:@^A-Z@:>@*=/!d;$qt_remove_sed_filter" $at_mfile`])
  AC_SUBST([QT_CXXFLAGS], ["$at_cv_env_QT_CXXFLAGS $at_cv_env_QT_DEFINES"])
