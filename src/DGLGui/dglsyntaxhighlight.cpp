...
#define FATAL \
    { throw std::runtime_error("Cannot parse syntax highlighting file."); }

class DGLHLTextCharFormat {
   public:
    DGLHLTextCharFormat(const QColor color, bool italics = false,
                        bool bold = false, bool strikeout = false) {
        ...
        if (color.isValid()) m_Format.setForeground(color);
    }
    DGLHLTextCharFormat(const QDomElement& xml) {
        ...
        if (xml.isNull() || xml.tagName() != "itemData" ||
            xml.attribute("defStyleNum", "") == "") {
            FATAL;
        }
...
class DGLHLRuleDetectChar : public DGLHLRuleBase {
   public:
    DGLHLRuleDetectChar(const DGLHLData* data, char _char, QString formatName,
                        QString actionName)
...
class DGLHLRuleDetect2Chars : public DGLHLRuleBase {
   public:
    DGLHLRuleDetect2Chars(const DGLHLData* data, char char1, char char2,
                          QString formatName, QString actionName)
            : DGLHLRuleBase(data, formatName, actionName
#ifdef HL_DEBUG
                            ,
...
class DGLHLRuleKeyword : public DGLHLRuleBase {
   public:
    DGLHLRuleKeyword(const DGLHLData* data, std::string _string,
                     QString formatName, QString actionName);
...
class DGLHLRuleInt : public DGLHLRuleBase {
   public:
    DGLHLRuleInt(const DGLHLData* data, QString formatName, QString actionName)
            : DGLHLRuleBase(data, formatName, actionName
#ifdef HL_DEBUG
                            ,
...
class DGLHLRuleHlCHex : public DGLHLRuleBase {
   public:
    DGLHLRuleHlCHex(const DGLHLData* data, QString formatName,
                    QString actionName)
            : DGLHLRuleBase(data, formatName, actionName
#ifdef HL_DEBUG
                            ,
...
class DGLHLRuleDecimal : public DGLHLRuleBase {
   public:
    DGLHLRuleDecimal(const DGLHLData* data, QString formatName,
                     QString actionName)
            : DGLHLRuleBase(data, formatName, actionName
#ifdef HL_DEBUG
                            ,
...
class DGLHLRuleHlCOct : public DGLHLRuleBase {
   public:
    DGLHLRuleHlCOct(const DGLHLData* data, QString formatName,
                    QString actionName)
            : DGLHLRuleBase(data, formatName, actionName
#ifdef HL_DEBUG
                            ,
...
class DGLHLRuleFloat : public DGLHLRuleBase {
   public:
    DGLHLRuleFloat(const DGLHLData* data, QString formatName,
                   QString actionName)
            : DGLHLRuleBase(data, formatName, actionName
#ifdef HL_DEBUG
                            ,
...
class DGLHLRuleStringDetect : public DGLHLRuleBase {
   public:
    DGLHLRuleStringDetect(const DGLHLData* data, QString _string,
                          QString formatName, QString actionName)
            : DGLHLRuleBase(data, formatName, actionName
#ifdef HL_DEBUG
                            ,
...
DGLHLRuleBase* DGLHLRuleBase::Create(const DGLHLData* data,
    ...
    if (xml.tagName() == "Detect2Chars") {
        ...
        return new DGLHLRuleDetect2Chars(data, char1[0], char2[0], formatName,
                                         actionName);
   ...
   public:
    ...
    HLResult doHighlight(const QString& str) const {
        ...
        for (size_t i = 0; i < m_rules.size(); i++) {
            ...
            if (pos < bestPos ||
                (pos == bestPos && matchSize > bestMatchSize)) {
                bestPos = pos;
...
#ifdef HL_DEBUG
            qDebug() << "Match(rule = " << m_rules[best]->m_debugRuleName
                     << ", pos=" << ret.pos << ",size=" << ret.size << "): |"
                     << str.mid(ret.pos, ret.size) << "|\n";
#endif
        } else {
            ret.pos = str.size();
            ret.size = 0;
    ...
    const DGLHLTextCharFormat* getDefaultFormat() const {
        return m_DefaultFormat;
    }

    const DGLHLActionBase* getLineEndAction() const {
        return m_LineEndAction.get();
    }
...
#ifdef HL_DEBUG
    qDebug() << "Action: stay, current context: "
             << state.getContext()->m_debugName << "\n";
#else
    (void)state;
#endif
}
...
void DGLHLActionPop::doAction(DGLSyntaxHighlighterGLSL::HLState& state) const {
    for (int i = 0; i < m_counter; i++) {
        ...
        state.pop();
#ifdef HL_DEBUG
        HL_DEBUG(qDebug() << "Action: pop, current context: "
                          << state.getContext()->m_debugName << "\n");
#endif
    }
}
    ...
    DGLHLData() : m_case_sensitive(true) {
        ...
        for (QDomElement element =
            ...
            for (QDomNode itemDataNode = element.firstChild();
                 !itemDataNode.isNull();
                 itemDataNode = itemDataNode.nextSibling()) {
...
DGLHLRuleKeyword::DGLHLRuleKeyword(const DGLHLData* data, std::string _string,
                                   QString formatName, QString actionName)
        : DGLHLRuleBase(data, formatName, actionName
#ifdef HL_DEBUG
                        ,
...
DGLHLActionBase* DGLHLActionBase::Create(const DGLHLData* data,
                                         const QString name) {
    if (name == "#stay") {
...
DGLHLRuleBase::DGLHLRuleBase(const DGLHLData* data, QString formatName,
                             QString actionName
#ifdef HL_DEBUG
                             ,
                             QString debugRuleName
...
DGLHLContext::DGLHLContext(const QDomElement& xml, const DGLHLData* data)
        : m_xml(xml) {
...
void DGLHLContext::link(const DGLHLData* data) {
    for (QDomNode node = m_xml.firstChild(); !node.isNull();
...
DGLSyntaxHighlighterGLSL::DGLSyntaxHighlighterGLSL(bool essl,
                                                   QTextDocument* _parent)
        : QSyntaxHighlighter(_parent), m_essl(essl) {
    if (!s_data) {
...
void DGLSyntaxHighlighterGLSL::highlightBlock(const QString& text) {
    ...
    while (pos < text.length()) {
        ...
        if (res.action) res.action->doAction(currentState);
