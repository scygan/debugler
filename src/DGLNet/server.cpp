...
template <>
struct type_dispatch<boost::asio::ip::tcp> {
    typedef boost::asio::ip::tcp::endpoint endpoint_t;
};
#ifdef BOOST_ASIO_HAS_LOCAL_SOCKETS
template <>
struct type_dispatch<boost::asio::local::stream_protocol> {
...
void Server<proto>::accept(bool wait) {
    if (wait) {
        ...
        ec = m_detail->m_acceptor.accept(Transport<proto>::m_detail->m_socket,
                                         ec);
        if (!ec) {
